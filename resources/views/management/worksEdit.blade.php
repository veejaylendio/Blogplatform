                ...
                <div class="card-body">
                    ...
                    <form action="{{route('works.update', $works->id)}}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            <label for="image" class="">File/Image Upload (<b style="color: #0a58ca">{{$works->works_image}}</b>)</label>
                            <input  name="works_image" type="file" id="image">
                        </div>
                        <div class="form-group">
                            <label for="title" class="required">Project Title</label>
                            <input name="works_title" type="text" id="title" placeholder="Ex: Fame" value="{{$works->works_title}}" required>
                        </div>
                        <div class="form-group">
                            <label for="type" class="required">Project Type</label>
                            <input name="works_project_type" type="text" id="type" placeholder="Ex: Branding" value="{{$works->works_project_type}}" required>
                        </div>
                        <div class="sm-form-group">
                            <label class="sm-label" for="urlInput">URL</label>
                            <input name="works_url" type="text" class="sm-input-text" id="urlInput" placeholder="Enter URL" value="{{$works->works_url}}">
                        </div>
                        <!-- Description field (added as requested) -->
                        <div class="form-group">
                            <label class="required" for="description">Description</label>
                            <textarea name="works_description" id="description" placeholder="Describe your Movies..." required>{{$works->works_description}}</textarea>
                        </div>
